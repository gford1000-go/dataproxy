...
func (b *baseHandler) zipData(data []byte, token string) ([]byte, error) {
	b.Debug(fmt.Sprintf("Page %v: GZipping", token))
	...
	_, err := zw.Write(data)
	if err != nil {
		b.Error(fmt.Sprintf("Page %v: GZip write error - %v", token, err))
		return nil, err
	}
	zw.Close()

	b.Debug(fmt.Sprintf("Page %v: GZipped", token))
...
func (b *baseHandler) unzipData(data []byte, token string) ([]byte, error) {
	b.Debug(fmt.Sprintf("Page %v: Ungzipping", token))

	buf := bytes.NewBuffer(data)

	zr, err := gzip.NewReader(buf)
	if err != nil {
		b.Error(fmt.Sprintf("Page %v: Zip buffer error - %v", token, err))
	...
	if err != nil {
		b.Error(fmt.Sprintf("Page %v: Zip read error - %v", token, err))
		return nil, fmt.Errorf("internal failure handling page (4)")
	}

	b.Debug(fmt.Sprintf("Page %v: Ungzipped", token))
	return data, nil
}

// writePage creates an encrypted page from the slice
func (b *baseHandler) writePage(data []byte, info *pageInfo) error {
	b.Debug(fmt.Sprintf("Page %v: Writing", info.token))
	defer b.Debug(fmt.Sprintf("Page %v: Completed", info.token))
	...
	if len(b.config.key) > 0 {
		b.Debug(fmt.Sprintf("Page %v: Encrypting", info.token))

		c, err := aes.NewCipher(b.config.key)
		if err != nil {
			b.Error(fmt.Sprintf("Page %v: Error creating Cipher - %v", info.token, err))
			return fmt.Errorf("invalid page data")
		}

		gcm, err := cipher.NewGCM(c)
		if err != nil {
			b.Error(fmt.Sprintf("Page %v: Error creating GCM - %v", info.token, err))
			return fmt.Errorf("internal failure creating page (1)")
		}

		nonce := make([]byte, gcm.NonceSize())
		if _, err = io.ReadFull(rand.Reader, nonce); err != nil {
			b.Error(fmt.Sprintf("Page %v: Error creating Nonce - %v", info.token, err))
			return fmt.Errorf("internal failure creating page (2)")
		}

		data = gcm.Seal(nonce, nonce, data, nil)

		b.Debug(fmt.Sprintf("Page %v: Completed encryption", info.token))
	}

	b.Debug(fmt.Sprintf("Page %v: Writing to Disk", info.token))
	fileName, err := b.createCacheFileName(info)
	if err != nil {
		b.Error(fmt.Sprintf("Page %v: Error writing to disk - %v", info.token, err))
		return err
	}
	err = ioutil.WriteFile(fileName, data, 0644)
	b.Debug(fmt.Sprintf("Page %v: Writing to Disk Completed", info.token))
	if err != nil {
		b.Error(fmt.Sprintf("Page %v: Error writing to disk - %v", info.token, err))
	}
...
func (b *baseHandler) retrievePage(info *pageInfo) (page []byte, err error) {
	b.Info(fmt.Sprintf("Page %v: Retrieving", info.token))
	defer b.Info(fmt.Sprintf("Page %v: Completed retrieval", info.token))

	pageLocation := b.getCacheFileName(info)

	b.Debug(fmt.Sprintf("Page %v: Reading from Disk", info.token))
	page, err = ioutil.ReadFile(pageLocation)
	b.Debug(fmt.Sprintf("Page %v: Reading from Disk completed", info.token))
	if err != nil {
		b.Error(fmt.Sprintf("Page %v: Error reading from disk - %v", info.token, err))
		return nil, fmt.Errorf("invalid request or page token")
	}

	// If a key is provided, assume the page is encrypted
	if len(b.config.key) > 0 {
		b.Debug(fmt.Sprintf("Page %v: Decrypting", info.token))
		...
		if len(page) < nonceSize {
			b.Error(fmt.Sprintf("Page %v: Data inconsistency", info.token))
		...
		if err != nil {
			b.Error(fmt.Sprintf("Page %v: Error decrypting - %v", info.token, err))
			return nil, fmt.Errorf("internal failure handling page (3)")
		}

		b.Debug(fmt.Sprintf("Page %v: Decrypted", info.token))
	}
